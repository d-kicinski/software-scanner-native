...
namespace scanner {
    ...
    void order_points(std::vector < cv::Point >
    const &pts,
...
std::optional <std::vector<cv::Point>> find_rect(std::vector < std::vector < cv::Point >>
const &contours)
{
if (contours.
...
return
std::make_optional(scan_contour);
}
